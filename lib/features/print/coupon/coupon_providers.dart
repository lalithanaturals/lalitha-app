...
final couponBranchesProvider = FutureProvider.autoDispose<List<Branch>>((ref) {
  return ref.watch(branchRepositoryProvider).listActive();
});

final couponStaffForBranchProvider = FutureProvider.autoDispose.family<List<Staff>, String>((ref, branchId) {
...
/// local state.
final couponsForBranchProvider = FutureProvider.autoDispose.family<List<Coupon>, String>((ref, branchId) {
  if (branchId.isEmpty) return Future.value(const []);
